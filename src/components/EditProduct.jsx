...
import { useEditProductMutation, useGetProductByIdQuery } from '../features/products/productsApi'
...
const EditProduct = () => {
  ...
  useEffect(() => {
    if (productById && !isLoadingGetProduct) {
      setNewProduct({
  ...
  const handleEditProduct = async (e) => {
    e.preventDefault();
    try {
      const response = await editProduct({ productId: id, newData: newProduct });
  ...
  return (
    <section className='px-2'>
      ...
      <div className='border p-3'>
        ...
        {isErrorEditProduct ?
          <p className='text-sm text-red-500 mb-2 border bg-red-100 p-2 rounded-md'>{errorEditProduct.data.message}</p>
          : ''
        }
